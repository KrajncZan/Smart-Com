...
client = mqtt.Client("client2",protocol=mqttv)
...
properties.ResponseTopic='org/responses/client2'
time.sleep(10) #wait to receive message
print("starting client 2")
count=1
while True:
    properties.CorrelationData=bytes([count])
    print("send count=",count)
    client.publish('org/common',"test from client2",properties=properties)
